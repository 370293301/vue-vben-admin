...
// 把原表格每列宽度同步到 totals 表的 col（或 th）
function syncTotalsWidths1() {
  const totalsTable = totalsTableRef.value;
  ...
  if (!sampleCell) sampleCell = srcCells[0] ?? null;

  // 复制背景/颜色
  const bgSource = getFirstNonTransparentAncestor(sampleCell ?? baseRow);
  // console.log('Found bg source:', bgSource, 'for sampleCell:', sampleCell)
  if (bgSource) {
    const cs = getComputedStyle(bgSource);
    if (cs.backgroundImage && cs.backgroundImage !== 'none') {
      totalsTable.style.backgroundImage = cs.backgroundImage;
      totalsTable.style.backgroundRepeat = cs.backgroundRepeat;
      totalsTable.style.backgroundPosition = cs.backgroundPosition;
      totalsTable.style.backgroundSize = cs.backgroundSize;
    } else if (cs.backgroundColor && cs.backgroundColor !== 'transparent') {
      totalsTable.style.background = cs.backgroundColor;
    } else {
...
function syncTotalsWidths() {
  ...
  if (bgInfo && bgInfo.cs) {
    const cs = bgInfo.cs;
    // 背景：优先 background-image，再 backgroundColor
...
// 多次重试以适配异步渲染（首次渲染可能比较慢）
function trySyncWithRetries(attempts = 0) {
  nextTick(() => {
    syncTotalsWidths();
    if (attempts < 8) {
      setTimeout(() => trySyncWithRetries(attempts + 1), 120);
    }
  });
}
...
function startThemeObserver() {
  ...
  // 监听 html / body class/attribute 变更（大多数主题开关会改这里）
