...
function normalizeGameDetailPayload(payload: any) {
  ...
  const records: GameDetailRecord[] = (Array.isArray(rawList) ? rawList : []).map((rec: any) => {
    // players 可能在 rec.listInfo / rec.players / rec.playerList 等
    const playersRaw = rec.listInfo ?? rec.players ?? rec.playerList ?? rec.playersInfo ?? [];
    const players: GameDetailPlayer[] = (Array.isArray(playersRaw) ? playersRaw : []).map((p: any) => ({
    ...
    return {
      // 兼容不同命名的时间字段
      time: rec.time ?? rec.createTime ?? rec.dt ?? rec.create_time ?? rec.date ?? '',
      // 房间号/roomKey 兼容
      roomKey: rec.roomKey ?? rec.roomNo ?? rec.roomId ?? rec.room_key ?? '',
      // 回放码
      recordCode: rec.recordCode ?? rec.replayCode ?? rec.record_code ?? rec.replay_code ?? '',
      players,
    };
  });

  const totalPages = Number(maybe.totalPages ?? maybe.total_pages ?? maybe.totalPage ?? maybe.total ?? 1);
...
export async function agentReqGameDetailRecord(opts: {
  ...
  console.log('[debug] agentReqGameDetailRecord ->', AGENT_REQ_GAME_DETAIL_RECORD_URL, body);

  const resp = await apiJavaPost(AGENT_REQ_GAME_DETAIL_RECORD_URL, body, JAVA_SECRET, {
    contentType: 'form',
...
export async function agentReqGameRecord(opts: {
  ...
  console.log('[debug] agentReqGameRecord body=', body, 'endpoint=', AGENT_REQ_GAME_RECORD_URL);
  ...
  return resp as unknown as { data?: AgentGameRecordResp; status?: number; [k: string]: any };
}
