...
import { apiJavaPost, apiNodeClientLogin } from '#/api/auth'; // 请确保 api/auth.ts 已按前文实现并导出这两个函数
...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    setToken(t: string) {
      ...
      if (t) {
        ...
        try {
          ...
          console.warn('[auth.setToken] set requestAxios header failed', error);
        }
        localStorage.setItem('TOKEN', t);
      } else {
        delete axios.defaults.headers.common.Authorization;
      ...
      try {
        console.log(
          '[auth.setToken] utils/http header =',
          axios.defaults.headers.common.Authorization,
        );
        console.log(
          '[auth.setToken] api/request header =',
          requestAxios.defaults.headers.common.Authorization,
        );
      } catch (error) {
        console.warn('[auth.setToken] print headers failed', error);
      }
    },
    /**
      ...
      try {
          ...
          if (menuItem.component) {
            if (typeof menuItem.component === 'string') {
              ...
              if (loader) route.component = loader;
                ...
                // 找不到视图时，保留 meta 但不报错
                console.warn(
                  '[registerMenus] view not found for',
                  menuItem.component,
                );
              }
            } else {
        ...
        for (const m of menus) {
          ...
          const exists = router
            ...
            .some(
              (r: any) =>
                String(r.name) === String(rr.name) || r.path === rr.path,
            );
          if (exists) {
            console.warn(
              '[registerMenus] skip duplicate route',
              rr.name,
              rr.path,
            );
            continue;
          ...
          try {
            if (router.getRoutes().some((r: any) => r.name === parentName)) {
              ...
              console.log(
                '[registerMenus] addRoute to parent',
                parentName,
                rr.path,
              );
            } else {
            ...
            console.error('[registerMenus] addRoute error', error, rr.path);
          }
        }
        ...
        console.log(
          '[registerMenus] routes registered, now getRoutes length:',
        ...
        console.log('[registerMenus] applied hideInMenu flags. allowedNames:', [
          ...allowedNames,
        ]);
        ...
        try {
          ...
          console.log(
            '[registerMenus] pushed /analytics, currentRoute:',
            router.currentRoute.value,
          );
        } catch (error) {
          console.warn(
            '[registerMenus] push /analytics failed or intercepted',
            error,
          );
        }
      } catch (error) {
        console.error('[registerMenus] failed', error);
      }
    },
    ...
    async authLogin(
      ...
      try {
        ...
        const sendPack: Record<string, any> = {
          Head: Number.parseInt('0xff03', 16), // 65283
        ...
        const nodeSuccess =
          nodeData && (nodeData.Head === 0 || nodeData.code === 0);
        if (!nodeSuccess) {
        ...
        const accountID =
          nodeBody?.AccountID ?? nodeBody?.accountID ?? nodeBody?.AccountId;
        const nodeToken =
          nodeBody?.Token ?? nodeBody?.token ?? nodeBody?.SDKToken;

        if (!accountID)
          throw new Error('Node login succeeded but no AccountID returned');
        ...
        const javaOk =
          jdata.Head === 0 || jdata.code === 0 || !!jdata.AccountID;
        if (!javaOk) {
          const msg =
            jdata?.Msg || jdata?.msg || jdata?.message || 'Java login failed';
          throw new Error(msg);
        }
        ...
        if (pid !== null && pid !== undefined) {
          ...
          try {
            ...
            console.warn('保存 AGENT_PID 失败', error);
          }
        }
        if (userStore) {
          userStore.setUserInfo(userInfo);
        ...
        const routerInst =
          router ?? (typeof window === 'undefined' ? undefined : useRouter());
        if (routerInst) {
          ...
          try {
            ...
            console.warn(
              '[authLogin] final push /analytics failed or intercepted',
              error,
            );
          }
          ...
          (function printRoutes(router) {
            ...
            const out = routes.map((r) => ({
              ...
              meta: r.meta || {},
              // children 数量
              childrenCount: Array.isArray((r as any).children)
                ? (r as any).children.length
                : 0,
          ...
          console.warn(
            '[authLogin] no router instance available, cannot register routes or push',
          );
        }
        ...
        try {
          if (
            access &&
            typeof access.setLoginExpired === 'function' &&
            access.loginExpired
          ) {
    ...
    async logout(arg?: boolean | { redirect?: boolean; router?: any }) {
      ...
      routerInst =
        routerInst ?? (typeof window === 'undefined' ? undefined : useRouter());
      ...
      try {
        ...
        if (routerInst && Array.isArray(menuNames) && menuNames.length > 0) {
          for (const nm of menuNames) {
            try {
              if (
                nm &&
                typeof routerInst.hasRoute === 'function' &&
                routerInst.hasRoute(nm)
              ) {
              ...
              console.warn('[logout] removeRoute error for', nm, error);
            }
          }
        }
      } catch (error) {
        console.warn('[logout] dynamic route cleanup error', error);
      }
      ...
      try {
        ...
        console.warn('[logout] resetAllStores error', error);
      }
      ...
      if (redirect && routerInst) {
        console.log(77_788_888_999);
        try {
          console.log(11_111_666_222_333);
          await routerInst.replace({ path: LOGIN_PATH });
        } catch (error) {
          console.warn(
            '[logout] router replace failed, fallback to location replace',
            error,
          );
